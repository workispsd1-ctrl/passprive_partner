...
export default function CashierOrdersPage() {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-100 font-semibold text-slate-900">Orders</div>
      {error ? <div className="px-4 py-3 text-sm text-rose-700">{error}</div> : null}
      {loading ? (
        <div className="px-4 py-6 text-sm text-slate-500">Loading...</div>
      ) : rows.length === 0 ? (
        <div className="px-4 py-6 text-sm text-slate-500">No direct cashier orders yet.</div>
      ) : (
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.map((o) => (
              <tr key={o.id}>
                ...
                <td className="px-3 py-2">T{o.table_no || "—"}</td>
                ...
                <td className="px-3 py-2">{String(o.booking_status || "PLACED")}</td>
              </tr>
            ))}
          </tbody>
        </table>
