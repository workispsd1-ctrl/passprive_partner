...
const STAFF_ROLE_OPTIONS = new Set(["restaurant_admin", "restaurant_cashier", "restaurant_kitchen", "restaurant_bearer"]);
...
async function getOwnerRestaurant(token, requestedRestaurantId = "") {
  ...
  const { data: ownerRestaurants, error: restErr } = await ownerQuery;
  ...
  if (reqId && ownedIds.has(reqId)) return { restaurantId: reqId, ownerUserId: userId };
  if (!reqId && ownedIds.size > 0) {
    const firstOwned = ownerRestaurants[0];
    if (firstOwned?.id) return { restaurantId: firstOwned.id, ownerUserId: userId };
  }
  ...
  if (reqId && managedIds.has(reqId)) return { restaurantId: reqId, ownerUserId: userId };
  if (!reqId && managedIds.size > 0) {
    const firstManaged = staffRows[0];
    if (firstManaged?.restaurant_id) return { restaurantId: firstManaged.restaurant_id, ownerUserId: userId };
  }
