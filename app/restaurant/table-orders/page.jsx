...
export default function TableOrdersPage() {
  ...
  async function updateStatus(orderId, nextStatus, fallbackStatus) {
    ...
    if (upErr) {
      ...
      setOrders((prev) =>
        prev.map((o) =>
          o.id === orderId ? { ...o, status: fallbackStatus, updated_at: new Date().toISOString() } : o
        )
      );
  ...
  async function onChangeStatus(order, nextStatus) {
    ...
    setLastStatusById((prev) => ({ ...prev, [order.id]: order.status }));
    await updateStatus(order.id, nextStatus, order.status);
  }

  async function onCancelDraft(order) {
    const rollbackStatus = lastStatusById[order.id] ?? order.status;
    setDraftStatusById((prev) => ({ ...prev, [order.id]: rollbackStatus }));

    if (rollbackStatus !== order.status) {
      await updateStatus(order.id, rollbackStatus, order.status);
    }
  }
      ...
      {orderedList.length === 0 ? (
        ...
        <div className="space-y-3">
          {orderedList.map((o) => {
            ...
            const draft = draftStatusById[o.id] ?? o.status;
            ...
            return (
              <div key={o.id} className="rounded-2xl border border-slate-200 bg-white p-4">
                ...
                <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
                  <label className="text-xs font-semibold text-slate-700 block mb-2">Change Status</label>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <select
                      ...
                      className="h-10 w-full sm:w-64 rounded-lg border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-700 outline-none focus:ring-2 focus:ring-amber-200 disabled:opacity-50"
                    >
                    ...
                    <button
                      ...
                      className="h-10 rounded-lg border border-slate-300 bg-white px-4 text-xs font-semibold text-slate-700 hover:bg-slate-100"
                    >
                      Cancel
                    </button>
                  </div>
