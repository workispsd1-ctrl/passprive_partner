...
// ✅ ALWAYS return OBJECT menu (never []), even if DB has []
      ...
      .map((s) => ({
        ...
        items: (Array.isArray(s.items) ? s.items : [])
          ...
          .map((i) => ({
            ...
            is_bestseller:
              typeof i.is_bestseller === "boolean" ? i.is_bestseller : false,
            is_available:
              typeof i.is_available === "boolean" ? i.is_available : true,
          })),
      })),
  };
}
...
export default function RestaurantMenuPage() {
  ...
  const saveMenu = async (nextMenu, forceRestaurantId) => {
    ...
    if (!data?.id) {
      setSaveError("Update returned no row (RLS is blocking UPDATE).");
      return;
    }
      ...
      if (editingSectionId) {
        next.sections = next.sections.map((s) =>
          s.id === editingSectionId
            ? { ...s, name, description: description || null }
            : s,
        );
      } else {
        const newSection = {
  ...
  const removeImage = (url) =>
    setItemImages((prev) => prev.filter((u) => u !== url));
  ...
  const openNewItem = () => {
    ...
    resetItemModal();
    editItemIdRef.current = uid(); // ✅ item id exists before upload
  ...
  // ✅ KEY FIX: after adding/editing item, SAVE to DB immediately
  const upsertItem = async () => {
    ...
    const nextMenu = (() => {
      const nextSections = (menu.sections || []).map((s) => {
        ...
        if (idx >= 0) items[idx] = { ...items[idx], ...nextPayload };
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <div className="text-sm text-gray-500">
            Manage sections and items for your restaurant menu
          </div>
          {saveError ? (
            <div className="mt-2 text-sm text-red-600">
              Save error: <span className="font-semibold">{saveError}</span>
            </div>
          ) : null}
        </div>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white overflow-hidden">
            ...
            {sections.length === 0 ? (
              <div className="px-4 py-10 text-center text-sm text-gray-500">
                Create your first section to start building your menu.
              </div>
            ) : (
        ...
        <div className="col-span-12 lg:col-span-8">
          <div className="rounded-2xl border border-gray-200 bg-white overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
              <div>
                <div className="text-sm font-semibold text-gray-900">
                  {selectedSection ? selectedSection.name : "Items"}
                </div>
                <div className="text-xs text-gray-500">
                  {selectedSection
            ...
            {!selectedSection ? (
              <div className="px-4 py-12 text-center text-sm text-gray-500">
                Select a section from the left to manage items.
              </div>
            ) : (selectedSection.items || []).length === 0 ? (
              <div className="px-4 py-12 text-center text-sm text-gray-500">
                No items in this section. Add your first item.
              </div>
            ) : (
              <div className="divide-y divide-gray-100">
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex items-start gap-3">
                          <div className="h-12 w-12 rounded-xl border border-gray-200 bg-gray-50 flex items-center justify-center overflow-hidden relative">
                            ...
                            {imgs.length > 1 ? (
                              <span className="absolute bottom-1 right-1 rounded-full bg-black/70 text-white text-[10px] px-1.5 py-0.5">
                                +{imgs.length - 1}
                              </span>
                            ) : null}
                          </div>
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            type="button"
                            onClick={() =>
                              toggleAvailability(selectedSection.id, it.id)
                            }
                            className="h-9 w-9 rounded-xl border border-gray-200 bg-white hover:bg-gray-50 flex items-center justify-center"
                          >
                            {it.is_available === false ? (
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              deleteItem(selectedSection.id, it.id)
                            }
                            className="h-9 w-9 rounded-xl border border-gray-200 bg-white hover:bg-red-50 flex items-center justify-center"
                          >
                            <Trash2 className="h-4 w-4 text-red-600" />
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Optional (shown below section name)"
              rows={3}
              className="mt-2 w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-gray-300"
            />
          </div>
            ...
            {itemImages.length > 0 ? (
              <div className="mt-3 grid grid-cols-4 gap-2">
                {itemImages.map((u) => (
                  <div
                    key={u}
                    className="relative h-16 rounded-xl border border-gray-200 overflow-hidden bg-gray-50"
                  >
                    <img
                      src={u}
                      alt="Item"
          ...
          <div className="col-span-12">
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className={`h-10 rounded-xl border px-4 text-sm font-semibold flex items-center gap-2 transition ${
                  itemVeg
