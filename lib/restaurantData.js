...
function parseMenuObject(value) {
...
function toMenuShape(urls = [], persistedMenu = null) {
  const persisted = parseMenuObject(persistedMenu) || {};
  const persistedUrls = [];
  if (Array.isArray(persisted.full_menu_image_urls)) persistedUrls.push(...persisted.full_menu_image_urls);
  if (typeof persisted.full_menu_image_url === "string" && persisted.full_menu_image_url) {
    persistedUrls.push(persisted.full_menu_image_url);
  }

  const clean = Array.from(
    new Set([...(Array.isArray(urls) ? urls : []), ...persistedUrls].map((u) => String(u || "").trim()).filter(Boolean))
  );
...
export async function fetchOwnedRestaurantBase(supabase, userId, select) {
  ...
  // Fallback: allow restaurant_admin staff accounts to manage one assigned restaurant.
  ...
  if (!staffRow?.restaurant_id) throw new Error("No owned restaurant found.");

  const { data: managedRestaurant, error: managedErr } = await supabase
    .from("restaurants")
    .select(select)
    .eq("id", staffRow.restaurant_id)
    .maybeSingle();
  if (managedErr) throw managedErr;
  if (!managedRestaurant) throw new Error("No managed restaurant found.");
...
export async function fetchOwnedRestaurantOffersData(supabase, userId) {
  const restaurant = await fetchOwnedRestaurantBase(supabase, userId, "id, menu_json");
  ...
  return {
    ...
    menu: toMenuShape(menuUrls, restaurant.menu_json),
    premiumAccess: {
      unlockAll,
...
export async function fetchRestaurantPublicMenuData(supabase, restaurantId) {
  const { data: restaurant, error } = await supabase
    .from("restaurants")
    .select("id, name, city, area, is_active, menu_json")
    .eq("id", restaurantId)
    .maybeSingle();
  if (error) throw error;
  ...
  return {
    ...restaurant,
    menu: toMenuShape(
      media.filter((row) => row.asset_type === "menu").map((row) => row.file_url),
      restaurant.menu_json
    ),
  };
}

export async function saveRestaurantMenuData(supabase, restaurantId, menu) {
  const payload = parseMenuObject(menu) || {};
  const { error } = await supabase
    .from("restaurants")
    .update({
      menu_json: {
        version: 1,
        updated_at: new Date().toISOString(),
        full_menu_image_url: payload.full_menu_image_url || null,
        full_menu_image_urls: Array.isArray(payload.full_menu_image_urls) ? payload.full_menu_image_urls : [],
